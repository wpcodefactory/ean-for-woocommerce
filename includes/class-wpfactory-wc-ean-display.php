...
if ( ! class_exists( 'WPFactory_WC_EAN_Display' ) ) :

class WPFactory_WC_EAN_Display {
	 ...
	 * @since   2.0.0
	 *
	 * @todo    (dev) Admin products list column: move to `class-wpfactory-wc-ean-display-admin.php` or `class-wpfactory-wc-ean-admin.php`?
	...
	function product_columns_style() {
		...
		wp_enqueue_style(
			'wpfactory-wc-ean-product-columns',
			wpfactory_wc_ean()->plugin_url() . '/assets/css/wpfactory-wc-ean-product-columns' . $min . '.css',
			array(),
	...
	function render_product_column_ean( $do_validate, $ean, $product_id = false ) {
		return ( $do_validate && ! wpfactory_wc_ean()->core->is_valid_ean( $ean, $product_id ) ? '<span style="color:red;">' . $ean . '</span>' : $ean );
	}
	...
	function render_product_columns( $column, $product_id ) {
		if ( 'ean' === $column ) {
			...
			if ( '' != ( $value = wpfactory_wc_ean()->core->get_ean( $product_id ) ) ) {
				$values[] = $this->render_product_column_ean( $do_validate, $value, $product_id );
			}
			if ( $product->is_type( 'variable' ) ) {
				foreach ( $product->get_children() as $child_id ) {
					if ( '' != ( $value = wpfactory_wc_ean()->core->get_ean( $child_id ) ) ) {
						$values[] = $this->render_product_column_ean( $do_validate, $value, $product_id );
					}
				}
			}
	...
	function add_ean_to_product_structured_data( $markup, $product ) {
		...
		$value = wpfactory_wc_ean()->core->get_ean( $product->get_id() );
		...
		if ( '' !== $value || apply_filters( 'alg_wc_ean_product_structured_data_allow_empty_value', false, $product ) ) {
			...
			if ( 'yes' === get_option( 'alg_wc_ean_frontend_product_structured_data_key_auto', 'yes' ) ) {
				$type = wpfactory_wc_ean()->core->get_type( $value, false, $product->get_id() );
				switch ( $type ) {
					case 'EAN8':
						$key = 'gtin8';
						break;
	...
	function variations_enqueue_scripts() {
		if ( 'product_meta' === get_option( 'alg_wc_ean_frontend_variation_position', 'product_meta' ) ) {
			$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
			wp_enqueue_script(
				'wpfactory-wc-ean-variations',
				wpfactory_wc_ean()->plugin_url() . '/assets/js/wpfactory-wc-ean-variations' . $min . '.js',
				array( 'jquery' ),
				wpfactory_wc_ean()->version,
				true
			);
			wp_localize_script(
				'wpfactory-wc-ean-variations',
				'wpfactory_wc_ean_variations_obj', array(
	...
	function variations_add_params( $args, $product = false, $variation = false ) {
		if ( $variation ) {
			$ean = $variation->get_meta( wpfactory_wc_ean()->core->ean_key );
			if ( 'product_meta' === get_option( 'alg_wc_ean_frontend_variation_position', 'product_meta' ) ) {
				$args['ean'] = $ean;
			} else { // Variation description
				if ( '' !== $ean ) {
					$args['variation_description'] .= str_replace( '%ean%', $ean, get_option( 'alg_wc_ean_template', wpfactory_wc_ean()->core->get_default_template() ) );
				}
			}
		}
		return $args;
	...
	function get_ean_output_data() {
		$result = array( 'do_output' => false, 'style' => '', 'value' => wpfactory_wc_ean()->core->get_ean() );
		if ( '' !== $result['value'] ) {
	...
	function get_ean_output_html( $ean, $template, $product_id = false, $wrapper_atts = '' ) {
		$template = wpfactory_wc_ean()->core->shortcodes->do_shortcode( $template, array( 'product_id' => $product_id ) );
	...
	function add_ean_single() {
		$this->add_ean(
			get_option( 'alg_wc_ean_template', wpfactory_wc_ean()->core->get_default_template() ),
			'single'
		);
	}
	...
	function add_ean_loop() {
		$this->add_ean(
			get_option( 'alg_wc_ean_template_loop', wpfactory_wc_ean()->core->get_default_template() ),
			'loop'
		);
	}
	...
	function add_ean_cart( $cart_item ) {
		...
		if ( $ean = wpfactory_wc_ean()->core->get_ean( $product_id ) ) {
			$template    = get_option( 'alg_wc_ean_frontend_cart_template', wpfactory_wc_ean()->core->get_default_template() );
...
return new WPFactory_WC_EAN_Display();
