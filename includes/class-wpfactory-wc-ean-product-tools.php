...
if ( ! class_exists( 'WPFactory_WC_EAN_Product_Tools' ) ) :

class WPFactory_WC_EAN_Product_Tools {
	...
	function bulk_actions_confirmation_js() {
		...
		wp_enqueue_script(
			'wpfactory-wc-ean-bulk-actions-confirmation',
			wpfactory_wc_ean()->plugin_url() . '/assets/js/wpfactory-wc-ean-bulk-actions-confirmation' . $min . '.js',
			array( 'jquery' ),
				...
				foreach ( $product_ids as $product_id ) {
					switch ( $action ) {
						case 'alg_wc_ean_generate':
							$result = (
									'' === get_post_meta( $product_id, wpfactory_wc_ean()->core->ean_key, true ) &&
									'' !== ( $ean = $this->generate_ean( $product_id, $data ) ) &&
									wpfactory_wc_ean()->core->set_ean( $product_id, $ean )
								);
						...
						case 'alg_wc_ean_delete':
							$result = delete_post_meta( $product_id, wpfactory_wc_ean()->core->ean_key );
							break;
					}
					if ( $result ) {
						$count++;
	 ...
	 * @todo    (dev) delete directly with SQL from the `meta` table: `$counter = $wpdb->query( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = '" . wpfactory_wc_ean()->core->ean_key . "'" );`
		...
		if ( 'yes' === get_option( 'alg_wc_ean_tool_delete_product_meta', 'no' ) ) {
			...
			if ( current_user_can( 'manage_woocommerce' ) ) {
				...
				foreach ( $this->get_products() as $product_id ) {
					if ( delete_post_meta( $product_id, wpfactory_wc_ean()->core->ean_key ) ) {
						$count++;
					}
				}
	...
	function product_on_insert_post( $post_id, $post, $update ) {
		...
		if (
			...
			$current_ean = get_post_meta( $post_id, wpfactory_wc_ean()->core->ean_key, true );
			...
			if ( '' === $current_ean ) {
				...
				if ( '' !== $ean ) {
					...
					wpfactory_wc_ean()->core->set_ean( $post_id, $ean );
				...
				switch ( $action ) {
					...
					case 'copy_to_variations':
						$meta_key = wpfactory_wc_ean()->core->ean_key;
	...
	function process_action_for_all_products( $action ) {
		...
		$count    = 0;
		foreach ( $products as $product_id ) {
			$current_ean = get_post_meta( $product_id, wpfactory_wc_ean()->core->ean_key, true );
			...
			if ( '' !== $ean && wpfactory_wc_ean()->core->set_ean( $product_id, $ean ) ) {
				if ( 'generate' === $action && '' !== $data['product_attribute'] ) {
					$this->add_product_attribute( $product_id, $ean, $data['product_attribute'] );
				}
				$count++;
	 ...
	 * @todo    (dev) move to a separate class/file, e.g., `class-wpfactory-wc-ean-generator.php`?
	 */
...
return new WPFactory_WC_EAN_Product_Tools();
