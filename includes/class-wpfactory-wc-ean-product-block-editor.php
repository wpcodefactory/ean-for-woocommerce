...
if ( ! class_exists( 'WPFactory_WC_EAN_Product_Block_Editor' ) ) :

class WPFactory_WC_EAN_Product_Block_Editor {
	...
	function add_block( $template_id, $template_area, $template ) {
		...
		$section->add_block(
			array(
				'id'         => 'wpfactory-wc-ean-product-block-editor-field',
	...
	function save_ean( $product, $request, $creating ) {
		if ( isset( $request['alg_wc_ean_product_block_editor'] ) ) {
			$product->update_meta_data( wpfactory_wc_ean()->core->ean_key, wc_clean( $request['alg_wc_ean_product_block_editor'] ) );
		}
		return $product;
	}
	...
	function get_ean( $response, $product, $request ) {
		$response->data['alg_wc_ean_product_block_editor'] = $product->get_meta( wpfactory_wc_ean()->core->ean_key );
		return $response;
	}
...
return new WPFactory_WC_EAN_Product_Block_Editor();
