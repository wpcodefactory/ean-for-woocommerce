...
if ( ! class_exists( 'WPFactory_WC_EAN_Order_Tools' ) ) :

class WPFactory_WC_EAN_Order_Tools {
	...
	function get_order_items_search( $ean ) {
		...
		ob_start();
		?>
		<div class="wpfactory-wc-ean-order-items-search-wrap">
			<p><?php printf(
				/* Translators: %s: EAN. */
			...
			if ( current_user_can( 'manage_woocommerce' ) ) {
				...
				foreach ( $this->get_orders() as $order_id ) {
					...
					foreach ( $order->get_items() as $item_id => $item ) {
						if ( wc_delete_order_item_meta( $item_id, wpfactory_wc_ean()->core->ean_key ) ) {
							$count++;
						}
					}
				}
	...
	function orders_add() {
		if ( 'yes' === get_option( 'alg_wc_ean_tool_orders_add', 'no' ) ) {
			...
			if ( current_user_can( 'manage_woocommerce' ) ) {
				$count = 0;
				foreach ( $this->get_orders() as $order_id ) {
					$count += wpfactory_wc_ean()->core->orders->add_ean_to_order_items_meta( $order_id );
				}
				if ( method_exists( 'WC_Admin_Settings', 'add_message' ) ) {
					WC_Admin_Settings::add_message(
						sprintf(
...
return new WPFactory_WC_EAN_Order_Tools();
