...
if ( ! class_exists( 'WPFactory_WC_EAN_Orders' ) ) :

class WPFactory_WC_EAN_Orders {
	...
	function set_order_item_meta_display_key( $display_key, $meta ) {
		return ( wpfactory_wc_ean()->core->ean_key === $meta->key ? get_option( 'alg_wc_ean_title', __( 'EAN', 'ean-for-woocommerce' ) ) : $display_key );
	}
	...
	function new_order_item_ajax( $item_id, $item ) {
		if (
			...
			'' === wc_get_order_item_meta( $item_id, wpfactory_wc_ean()->core->ean_key ) &&
			( $product_id = ( ! empty( $item['variation_id'] ) ? $item['variation_id'] : $item['product_id'] ) ) &&
			'' !== ( $ean = wpfactory_wc_ean()->core->get_ean( $product_id, true ) )
		) {
			wc_update_order_item_meta( $item_id, wpfactory_wc_ean()->core->ean_key, $ean );
		}
	}
	 ...
	 * @todo    (dev) `( $do_overwrite || '' === wc_get_order_item_meta( $item_id, wpfactory_wc_ean()->core->ean_key, true )`
	 */
...
return new WPFactory_WC_EAN_Orders();
