...
if ( ! class_exists( 'WPFactory_WC_EAN_Settings_Barcodes_Compatibility' ) ) :

class WPFactory_WC_EAN_Settings_Barcodes_Compatibility extends WPFactory_WC_EAN_Settings_Section {
		...
		$settings = array(
			array(
				...
				'desc'     => sprintf(
					...
					'<a href="' . admin_url( 'admin.php?page=wc-settings&tab=wpfactory_wc_ean&section=' . ( '1d' === $this->dim ? 'barcodes' : 'barcodes_2d' ) ) . '">' .
				...
				array(
					'desc'     => sprintf(
						...
						'<pre style="background-color:#dfdfe0; padding:3px 5px 2px; width: fit-content;">' .
							'action=wpfactory_wc_ean_print_barcodes_to_pdf' . PHP_EOL .
							'section=print' .
						'</pre>'
					),
