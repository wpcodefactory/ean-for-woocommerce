...
if ( ! class_exists( 'WPFactory_WC_EAN_Shortcodes' ) ) :

class WPFactory_WC_EAN_Shortcodes {
	...
	function ean_is_unique_shortcode( $atts, $content = '' ) {
		...
		$result = '';
		if ( ! empty( $ean ) ) {
			$result = ( ! wpfactory_wc_ean()->core->do_ean_exist( $ean, $product_id ) ? $atts['unique_text'] : $atts['duplicate_text'] );
		}
	...
	function ean_is_valid_shortcode( $atts, $content = '' ) {
		...
		$result = '';
		if ( ! empty( $ean ) ) {
			$result = ( wpfactory_wc_ean()->core->is_valid_ean( $ean, $product_id ) ? $atts['valid_text'] : $atts['invalid_text'] );
		}
	...
	function ean_shortcode( $atts, $content = '' ) {
		...
		foreach ( $product_ids as $product_id ) {
			$result[] = wpfactory_wc_ean()->core->get_ean( $product_id );
		}
...
return new WPFactory_WC_EAN_Shortcodes();
