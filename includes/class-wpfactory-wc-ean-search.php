...
if ( ! class_exists( 'WPFactory_WC_EAN_Search' ) ) :

class WPFactory_WC_EAN_Search {
	 ...
	 * @todo    (dev) Flatsome to `class-wpfactory-wc-ean-compatibility.php`?
	...
	function flatsome_search_ajax( $search_query, $args, $defaults ) {
		if ( ! $this->flatsome_theme_mod_search_by_sku ) {
			$args['meta_query'][0]['key']     = wpfactory_wc_ean()->core->ean_key;
	...
	function flatsome_search_ajax_func( $func, $search_query, $args, $defaults ) {
		return ( ! empty( $args['meta_query'][0]['key'] ) && '_sku' === $args['meta_query'][0]['key'] ? 'wpfactory_wc_ean_flatsome_search_ajax' : $func );
	}
	...
	function search_backend( $query ) {
		self::_search_backend( $query, wpfactory_wc_ean()->core->ean_key, $this );
	}
	...
	function search( $wp_query ) {
		self::_search_frontend( $wp_query, wpfactory_wc_ean()->core->ean_key );
	}
...
if ( ! function_exists( 'wpfactory_wc_ean_flatsome_search_ajax' ) ) {
	/**
	 * wpfactory_wc_ean_flatsome_search_ajax.
	 *
	 * @version 5.5.6
	 * @since   2.1.0
	 */
	function wpfactory_wc_ean_flatsome_search_ajax( $search_query, $args, $defaults ) {
		return wpfactory_wc_ean()->core->search->flatsome_search_ajax( $search_query, $args, $defaults );
	}
}

return new WPFactory_WC_EAN_Search();
