...
if ( ! class_exists( 'WPFactory_WC_EAN_REST_API' ) ) :

class WPFactory_WC_EAN_REST_API {
	...
	function product_search( $args, $request ) {
		if ( isset( $request['ean'] ) ) {
			...
			// Meta key/value
			$args['meta_key']   = wpfactory_wc_ean()->core->ean_key; // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
	...
	function product_add_ean( $response, $product, $request ) {
		...
		$response->data[ $res_key ] = wpfactory_wc_ean()->core->get_ean( $product->get_id() );
		return $response;
	}
		...
		foreach ( $response->data['line_items'] as $item_key => &$item ) {
			...
			if ( ! empty( $item['meta_data'] ) ) {
				foreach ( $item['meta_data'] as $meta_data ) {
					if ( isset( $meta_data['key'], $meta_data['value'] ) && wpfactory_wc_ean()->core->ean_key === $meta_data['key'] ) {
...
return new WPFactory_WC_EAN_REST_API();
